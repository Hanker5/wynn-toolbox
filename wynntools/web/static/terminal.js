...
(() => {
  ...
  const toolRows = new Map();      // id -> row, kept in place so the bar animates
  function toolRow() {
  ...
  function showTools(tools) {
    ...
    for (const t of tools) {
      // Rows are updated in place: re-inserting one would restart the bar's animation.
      ...
      // "~" on a percentage worked out from how long this command usually takes.
      stat.textContent = `${t.estimated ? "~" : ""}${pct}% · ${clock(t.elapsed)}`;
      bar.setAttribute("aria-label", t.label);
      bar.setAttribute("aria-valuenow", String(pct));
      fill.style.width = `${t.fraction * 100}%`;
      cmd.textContent = t.detail ? `${t.command} · ${t.detail}` : t.command;
      row.title = t.estimated ? `${cmd.textContent}\n(estimated from past runs)` : cmd.textContent;
    }
